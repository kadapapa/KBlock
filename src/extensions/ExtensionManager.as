...
public class ExtensionManager {
	...
	public function ExtensionManager(app:KBlock) {
		this.app = app;
...
//			case "ICbrick":
	...
	private function refreshList():void{
		_extensionList = [];
		if(ApplicationManager.sharedManager().documents.resolvePath("KBlock/libraries/").exists){
			var docs:Array =  ApplicationManager.sharedManager().documents.resolvePath("KBlock/libraries/").getDirectoryListing();
	...
	*/
	public function copyLocalExtensionFiles():void{
		...
		for each(var sf:File in srcFile.getDirectoryListing()){
			var tf:File = ApplicationManager.sharedManager().documents.resolvePath("KBlock/libraries/"+sf.name);
			if(!tf.exists){
				sf.copyTo(tf,true);
			}else{
...
//		}
	...
	private function copyFirmwareAndHex():void{
		/*
		var srcFile:File = File.applicationDirectory.resolvePath("firmware/kblock_firmware/");
		var tf:File = ApplicationManager.sharedManager().documents.resolvePath("KBlock/firmware/kblock_firmware/");
		srcFile.copyTo(tf,true);
		var botFile:File = File.applicationDirectory.resolvePath("firmware/mbot_firmware/");
		var bottf:File = ApplicationManager.sharedManager().documents.resolvePath("KBlock/firmware/mbot_firmware/");
		botFile.copyTo(bottf,true);
		var hexFile:File = File.applicationDirectory.resolvePath("tools/hex/");
		var htf:File = ApplicationManager.sharedManager().documents.resolvePath("KBlock/tools/hex/");
		hexFile.copyTo(htf,true);
		var localsFile:File = File.applicationDirectory.resolvePath("locale/");
		var ltf:File = ApplicationManager.sharedManager().documents.resolvePath("KBlock/locale/");
	...
	static private function copyDir(dirName:String, destDirName:String=null):void
	{
		...
		var toFile:File = File.applicationStorageDirectory.resolvePath("KBlock").resolvePath(destDirName || dirName);
...
//								KBlock.app.runtime.exitRequest();
							//}
						}
					}
				}
...
//		parseAllTranslators();
		KBlock.app.translationChanged();
		KBlock.app.updatePalette();
...
//			KBlock.app.runtime.enterRequest();
...
//			b.nextID.push(ext.nextID);
			...
			var objs:Array = KBlock.app.extensionManager.specForCmd(ext.name+"."+op);
