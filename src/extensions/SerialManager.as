...
//			_avrdudeConfig = _isMacOs?"avrdude_mac.conf":"avrdude.conf";
		...
		public function setKBlock(KBlock:KBlock):void{
			_KBlock = KBlock;
		}
		...
		public function update():void{
			if(!_serial.isConnected){
				KBlock.app.topBarPart.setDisconnectedTitle();
				return;
			}else{
				KBlock.app.topBarPart.setConnectedTitle("Serial Port");
			}
		}
		...
		public function upgrade(hexFile:String=""):void{
			...
			KBlock.app.track("/OpenSerial/Upgrade");
			executeUpgrade();
...
//			if(DeviceManager.sharedManager().currentDevice.indexOf("leonardo")>-1){
			if(((DeviceManager.sharedManager().currentDevice.indexOf("leonardo")>-1)) ||
			   ((DeviceManager.sharedManager().currentDevice.indexOf("kadapapa")>-1))) {
				_serial.close();
		...
		public function openSource():void{
			KBlock.app.track("/OpenSerial/ViewSource");
			var file:File = ApplicationManager.sharedManager().documents.resolvePath("KBlock/firmware/" + getFirmwareName());
		...
		static private function getFirmwareName():String
		{
			...
			if(boardName == "icbrick"){
				return "icbrick_firmware";
			}
			if(boardName == "kadapapa"){
				return "kadapapa_firmware";
			}
//			return "orion_firmware";
			return "icbrick_firmware";
		}
		public function disconnect():void{
			currentPort = "";
			KBlock.app.topBarPart.setDisconnectedTitle();
//			KBlock.app.topBarPart.setBluetoothTitle(false);
			ArduinoManager.sharedManager().isUploading = false;
			_serial.close();
			_serial.removeEventListener(Event.CHANGE,onChanged);
//			_serial.dispose();
		...
		public function reconnectSerial():void{
			...
			*/
//			_serial.close();
//			setTimeout(function():void{connect(_selectPort);},3000);
		}
...
//			if(NativeProcess.isSupported) {
				...
				if(currentDevice=="leonardo"){
					...
					v.push("-U");
					if(_hexToDownload.length==0){
						var hexFile_mega1280:String = (ApplicationManager.sharedManager().documents.nativePath+"/KBlock/tools/hex/mega1280.hex");//.split("\\").join("/");
					...
					v.push("-U");
					if(_hexToDownload.length==0){
						var hexFile_mega2560:String = (ApplicationManager.sharedManager().documents.nativePath+"/KBlock/tools/hex/mega2560.hex");//.split("\\").join("/");
					...
					v.push("-U");
					if(_hexToDownload.length==0){
						var hexFile_nano328:String = (ApplicationManager.sharedManager().documents.nativePath+"/KBlock/tools/hex/nano328.hex");//.split("\\").join("/");
					...
					v.push("-U");
					if(_hexToDownload.length==0){
						var hexFile_nano168:String = (ApplicationManager.sharedManager().documents.nativePath+"/KBlock/tools/hex/nano168.hex");//.split("\\").join("/");
					...
					v.push("-U");
					if(_hexToDownload.length==0){
						var hexFile_ic:String = getHexFilePath();
						v.push("flash:w:"+hexFile_ic+":i");
						tf = new File(hexFile_ic);
					}else{
						v.push("flash:w:"+_hexToDownload+":i");
						tf = new File(_hexToDownload);
					}
				}else if(currentDevice=="kadapapa"){
					v.push("-patmega32u4");
					v.push("-cavr109");
					v.push("-P"+currentPort);
					v.push("-b57600");
					...
					v.push("-U");
					if(_hexToDownload.length==0){
						var hexFile_kadapapa:String = getHexFilePath();
						tf = new File(hexFile_kadapapa);
						v.push("flash:w:"+hexFile_kadapapa+":i");
					}else{
						tf = new File(_hexToDownload);
						v.push("flash:w:"+_hexToDownload+":i");
					}
...
//			if(board.indexOf("_uno") > 0 || board.indexOf("_ic")){
				if(board.indexOf("mbot") >= 0){
					fileName = "mbot";
				}else if(board.indexOf("shield") >= 0){
					fileName = "shield";
				}else if(board.indexOf("icbrick") >= 0){
					fileName = "icbrick";
				}else{
					fileName = "uno";
				}
				...
				if(board.indexOf("baseboard") >= 0){
					fileName = "baseboard";
				}else if(board.indexOf("kadapapa") >= 0){
					fileName = "kadapapa";
				}else{
					fileName = "leonardo";
				}
...
//				throw new Error(board);
			...
			return ApplicationManager.sharedManager().documents.nativePath + "/KBlock/tools/hex/" + fileName + ".hex";
		}
		/*
...
//			_dialog.setText(Translator.map('Executing')+" ... "+_upgradeBytesLoaded+"%");
		...
		*/
		...
		private function onExit(event:NativeProcessExitEvent):void
		{
			...
			setTimeout(function():void{connect(_selectPort);},2000);
			setTimeout(function():void{connect(_selectPort);},2200);
//			disconnect();
...
//			reconnectSerial();
		...
		*/
		public function executeUpgrade():void {
			...
			_dialog.showOnStage(_KBlock.stage);
		}
	}
}