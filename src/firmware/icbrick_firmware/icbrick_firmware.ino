/*************************************************************************
* File Name          : ICbrick_Firmware.ino
* Author             : Ander
* Updated            : Ander
* Version            : V1.10101
* Date               : 03/06/2014
...
KA_DCMotor dcmotor;
KA_Temperature ts;
KA_UltrasonicSensor us;
KA_Digitron dig;
KA_Buzzer buzzer;
//KA_IR ir;
KA_Gyro gyro;
KA_Port generalDevice;
//KA_Joystick joystick;
//KA_Compass Compass;
KA_Humiture humiture;
//KA_FlameSensor FlameSensor;
//KA_GasSensor GasSensor;
//KA_TouchSensor touchSensor;
//KA_4Button buttonSensor;

typedef struct KA_Module
{
    int device;
    int port;
    int slot;
...
void parseData()
{
	/**************************************************
		...
		0      1      2      3      4      5      6      7      8
		0xff   0x55   0x06   0x00   0x02   0x22   0x09   0x00   0x00   0x0a
    ***************************************************/
    isStart = false;
...
void runModule(int device)
{
    ...
    ***************************************************/
    ...
    case RGBLED:
    {
        KA_RGBLed rgbled(port);
        uint8_t index = readBuffer(7);
        uint8_t red   = readBuffer(8);
        uint8_t green = readBuffer(9);
        uint8_t blue  = readBuffer(10);
    ...
    case SERVO:
    {
        if(generalDevice.getPort() != port)
        {
            ...
            servos.attach(generalDevice.pin1());
        }
        int v = readBuffer(7);
        if(v >= 0 && v <= 180)
        {
    ...
    break;
        ...
        case JOYSTICK:{
          ...
          dcmotor.reset(M1);
          dcmotor.run(leftSpeed);
          int rightSpeed = readShort(8);
          dcmotor.reset(M2);
          dcmotor.run(rightSpeed);
         }
        break;
        case IR:{
          String Str_data;
...
void readSensor(int device)
{
    /**************************************************
        ff     55     len    idx    action device port   slot   data   a
        0      1      2      3      4      5      6      7      8
        0xff   0x55   0x04   0x03   0x01   0x01   0x01   0x0a   0x0a   0x0a
    ***************************************************/
    float value = 0.0;
    int port, slot, pin;
    port = readBuffer(6);
