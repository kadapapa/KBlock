package extensions
{
	...
	public class ArduinoManager extends EventDispatcher
	{
		...
		private var leoPortMap:Array=[[null,null],["12","A5"],["SCK","A4"],["5","A3"],["6","A2"],["11","A1"],["13","A0"],["9","7"],["10","4"],[null,null],[null,null],[null,null],["8",null],["13",null]]
	...
	if(str!=NULL && strcmp(str,varName)==0){
	  ...
	  *var = v;
	  lineParsed = true;
		...
		public function setScratch(scratch:KBlock):void{
			_scratch = scratch;
		}
		...
		private function getCodeBlock(blk:Object,params:Array=null):CodeBlock{
			...
			else if(blk[0]=="writeVariable:")
			{
				...
				var objs:Array = KBlock.app.extensionManager.specForCmd(blk[0]);
				if(objs!=null){
					var obj:Object = objs[objs.length-1];
					obj = obj[obj.length-1];
					if(typeof obj == "object"){
						var ext:ScratchExtension = KBlock.app.extensionManager.extensionByName(blk[0].split(".")[0]);
		...
		private function parseModules(blks:Object):void{
			...
			for(var i:int;i<blks.length;i++){
				var b:Object = blks[i];
				var objs:Array = KBlock.app.extensionManager.specForCmd(blks[0]);
				if(objs!=null){
					var obj:Object = objs[objs.length-1];
					obj = obj[obj.length-1];
		...
		private function parseCodeBlocks(blks:Object):Boolean{
			...
			for(var i:int;i<blks.length;i++){
				...
				if(b[0].indexOf("runArduino")>-1){
					...
					var objs:Array = KBlock.app.extensionManager.specForCmd(blks[0]);
					var n:String = blks[0];
					var ext:ScratchExtension = KBlock.app.extensionManager.extensionByName(n.split(".")[0]);
					if(ext!=null){
						if(srcDocuments.indexOf(ext.srcPath)==-1){
							srcDocuments.push(ext.srcPath);
						}
		...
		private function buildDefine():String{
			...
			for(var i:int=0;i<varList.length;i++){
				...
				var code:* = StringUtil.substitute("byte {0};\n" ,castVarName(v))
		...
		private function prepareProjectDir(ccode:String):void{
			...
			//			var srcdir:File = ApplicationManager.sharedManager().documents.resolvePath("KBlock/libraries/"+_extSrcPath+"/src");
		...
		public function get projectDocumentName():String{
			...
			var pName:String = KBlock.app.projectName().split(" ").join("").split("(").join("").split(")").join("");
		...
		public function buildAll(ccode:String):String
		{
			...
			dialog.showOnStage(KBlock.app.stage);
		...
		private function buildArduinoLib(buildDir:File):void{
			...
			listArduinoLib(new File(arduinoInstallPath+arduinoLibPath+"/Wire/src"))
			listArduinoLib(new File(arduinoInstallPath+arduinoLibPath+"/Wire/src/utility"))
			//			var servoFile:File = new File(arduinoInstallPath+"/libraries/Servo/src/avr");
			//			var servoFile:File = new File(arduinoInstallPath+"/libraries/Servo/src");
			//support 1.6.5
			//			if(servoFile.exists){
			//				listArduinoLib(new File(arduinoInstallPath+"/libraries/Servo/src/avr"));
			//				listArduinoLib(new File(arduinoInstallPath+"/libraries/Servo/src"));
			//			}else{
			//				listArduinoLib(new File(arduinoInstallPath+"/libraries/Servo"));
			//			}
			
			listArduinoLib(new File(arduinoInstallPath+arduinoLibPath+"/SoftwareSerial/src"));
			
			if(_currentDevice=="uno" || _currentDevice=="leonardo"){
				listArduinoLib(new File(arduinoInstallPath+"/libraries/makeblock"));
			}
			if(_currentDevice=="icbrick"){
				listArduinoLib(new File(arduinoInstallPath+"/libraries/Kblock"));
			}
			if(_currentDevice=="kadapapa"){
				listArduinoLib(new File(arduinoInstallPath+"/libraries/Kblock/src"));
			}
...
//			var file:File = new File(arduinoInstallPath+"/hardware/tools/avr/bin/avr-ar"+(ApplicationManager.sharedManager().system==ApplicationManager.WINDOWS?".exe":"")); 
			var file:File = new File(arduinoInstallPath+"/hardware/tools/avr/bin/avr-gcc-ar"+(ApplicationManager.sharedManager().system==ApplicationManager.WINDOWS?".exe":""));
		...
		private function compileCpp(cpp:String,dir:File):void{
			...
			}else if(_currentDevice=="icbrick"){
				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -MMD -mmcu=atmega328p -DF_CPU=16000000L -DARDUINO=10605 -DARDUINO_AVR_UNO -DARDUINO_ARCH_AVR -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/standard -I"+path+"/libraries/Servo/src -I"+path+"/libraries/Servo -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial -I" + path+"/libraries/kblock/src"
			}else if(_currentDevice=="kadapapa"){
//				cmd = " -c -g -Os -w -fno-exceptions -ffunction-sections -fdata-sections -MMD -mmcu=atmega32u4 -DF_CPU=16000000L -DARDUINO=10605 -DARDUINO_AVR_LEONARDO -DARDUINO_ARCH_AVR -DUSB_VID=0x2341 -DUSB_PID=0x8036 -DUSB_MANUFACTURER=\"Unknown\" -DUSB_PRODUCT=\"Arduino Leonardo\" -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/leonardo -I"+path+"/libraries/Servo/src -I"+path+"/libraries/Servo -I"+path+arduinoLibPath+"/Wire/src -I"+path+arduinoLibPath+"/Wire/utility -I"+path+arduinoLibPath+"/SoftwareSerial/src -I" + path+"/libraries/Kblock/src"
//				cmd = " -c -g -Os -Wall -Wextra -std=gnu++11 -fpermissive -fno-exceptions -ffunction-sections -fdata-sections -fno-threadsafe-statics -MMD -flto -mmcu=atmega32u4 -DF_CPU=16000000L -DARDUINO=10800 -DARDUINO_AVR_LEONARDO -DARDUINO_ARCH_AVR -DUSB_VID=0x2341 -DUSB_PID=0x8036 -DUSB_MANUFACTURER=\"Unknown\" -DUSB_PRODUCT=\"Arduino Leonardo\" -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/leonardo -I"+path+arduinoLibPath+"/Wire/src -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/SoftwareSerial/src -I"+path+"/libraries/Kblock/src"							
				cmd = " -c -g -Os -Wall -Wextra -std=gnu++11 -fpermissive -fno-exceptions -ffunction-sections -fdata-sections -fno-threadsafe-statics -MMD -flto -mmcu=atmega32u4 -DF_CPU=16000000L -DARDUINO=10800 -DARDUINO_AVR_Jupiter -DARDUINO_ARCH_AVR -DUSB_VID=0x2341 -DUSB_PID=0x8053 -DUSB_MANUFACTURER=\"Unknown\" -DUSB_PRODUCT=\"Kadapapa Jupiter\" -I"+path+avrPath+"/cores/arduino -I"+path+avrPath+"/variants/leonardo -I"+path+arduinoLibPath+"/Wire/src -I"+path+arduinoLibPath+"/Wire -I"+path+arduinoLibPath+"/SoftwareSerial/src -I"+path+"/libraries/Kblock/src"
			}		
		...
		private function gotoNextNativeCmd(event:Event):void{
			...
			if(nativeWorkList.length>0 && compileErr==false){
				...
				KBlock.app.scriptsPart.appendMessage(nativeProcessStartupInfo.executable.nativePath)
				KBlock.app.scriptsPart.appendMessage(nativeProcessStartupInfo.arguments.toString())
				process.start(nativeProcessStartupInfo); 
		...
		{ 
			...
			KBlock.app.scriptsPart.appendMessage(""+(date.month+1)+"-"+date.date+" "+date.hours+":"+date.minutes+": Got: "+output); 
			*/
		}
		...
		private function onExit(event:NativeProcessExitEvent):void
		{
			...
			KBlock.app.scriptsPart.appendMessage(""+(date.month+1)+"-"+date.date+" "+date.hours+":"+date.minutes+": Process exited with "+event.exitCode);
			numOfSuccess++;
			if(event.exitCode > 0){
				KBlock.app.scriptsPart.appendMsgWithTimestamp(errorText, true);
				errorText = null;
			}
			if(compileErr == false){
				dispatchEvent(new Event(EVENT_NATIVE_DONE));
