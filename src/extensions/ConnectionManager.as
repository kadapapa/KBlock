	...
	public class ConnectionManager extends EventDispatcher
	{
		...
		public function onConnect(name:String):void{
			switch(name){
				...
				case "reset_program":{
					SerialManager.sharedManager().upgrade(ApplicationManager.sharedManager().documents.resolvePath("KBlock/tools/hex/kadapapa.hex").nativePath);
					break;
				}
...
/**					
				case "driver":{
					KBlock.app.track("/OpenSerial/InstallDriver");
					if(ApplicationManager.sharedManager().system==ApplicationManager.MAC_OS){
						...
						navigateToURL(new URLRequest("https://www.kadapapa.com/Makeblock-USB-Driver"));
					}else{
						//						var fileDriver:File = new File(File.applicationDirectory.nativePath+"/drivers/Driver_for_Windows.exe");
						//						var fileDriver32:File = new File(File.applicationDirectory.nativePath+"/drivers/KaDriver/dpinst-x86.exe");
						//						fileDriver32.openWithDefaultApplication();						
						var fileDriver64:File = new File(File.applicationDirectory.nativePath+"/drivers/KaDriver/dpinst-amd64.exe");
						fileDriver64.openWithDefaultApplication();
					}
					break;
				}
*/
				case "driver32":{
					...
					if(ApplicationManager.sharedManager().system==ApplicationManager.MAC_OS){
						...
						var fileDriver32:File = new File(File.applicationDirectory.nativePath+"/Arduino/drivers/dpinst-x86.exe");
						fileDriver32.openWithDefaultApplication();						
					}
					break;
				}
				case "driver64":{
					...
					if(ApplicationManager.sharedManager().system==ApplicationManager.MAC_OS){
						...
						var fileDriver64:File = new File(File.applicationDirectory.nativePath+"/Arduino/drivers/dpinst-amd64.exe");
						fileDriver64.openWithDefaultApplication();
					}
					break;
				}
		...
		public function onClose(port:String):void{
			...
			if(!SerialDevice.sharedDevice().connected){
				...
				if(SerialManager.sharedManager().isConnected||HIDManager.sharedManager().isConnected||BluetoothManager.sharedManager().isConnected){
					...
					KBlock.app.topBarPart.setConnectedTitle("Network");
				}
			}
		...
		public function onReceived(bytes:ByteArray):void{
			_bytes = bytes;
			KBlock.app.scriptsPart.onSerialDataReceived(bytes);
