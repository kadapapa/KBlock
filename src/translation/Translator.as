...
//	private static const font13:Array = ['ar'];
	...
	public static function initializeLanguageList():void {
		...
		languages = KBlock.app.server.getLanguageList();
	...
	public static function setLanguage(lang:String):void {
		...
		setFontsFor(lang);
		if ('en' == lang){
			KBlock.app.translationChanged(); // there is no .po file English
		}else {
			var data:Object = KBlock.app.server.getPOFile(lang);
			if (data) {
				dictionary = data;
				checKBlockTranslations();
				setFontsFor(lang);
				KBlock.app.extensionManager.parseAllTranslators();
			}
			setTimeout(KBlock.app.translationChanged, 0);
//			KBlock.app.translationChanged();
		}
		langChangedSignal.dispatchEvent(new Event(Event.CHANGE));

		KBlock.app.server.setSelectedLang(lang);
	}
	...
	private static function fontSizeMenu():void {
		...
		m.showOnStage(KBlock.app.stage);
	}
...
//		}
	...
	private static function checKBlockSpec(spec:String):void {
		...
		if (!argsMatch(extractArgs(spec), extractArgs(translatedSpec))) {
			KBlock.app.log('Block argument mismatch:');
			KBlock.app.log('    ' + spec);
			KBlock.app.log('    ' + translatedSpec);
