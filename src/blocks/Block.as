...
//		KBlock.app.runtime.checkForGraphicEffects();
	}
	...
	public function insertBlockAbove(b:Block):void {
		...
		parent.addChild(b);
		b.bottoKBlock().insertBlock(this);
	}
	...
	public function bottoKBlock():Block {
...
/*
	public function showHelp():void {
		...
		if((i = op.indexOf('.')) > -1) {
			...
			if(KBlock.app.extensionManager.isInternal(extName))
				KBlock.app.showTip('ext:'+extName);
			else
				DialogBox.notify('Help Missing', 'There is no documentation available for experimental extension "'+extName+'".', KBlock.app.stage);
		}
		else {
			KBlock.app.showTip(op);
		}
	}
*/
	public function duplicateStack(deltaX:Number, deltaY:Number):void {
		if (isProcDef()) return; // don't duplicate procedure definition
		var forStage:Boolean = KBlock.app.viewedObj() && KBlock.app.viewedObj().isStage;
		var newStack:Block = BlockIO.stringToStack(BlockIO.stackToString(this), forStage);
		newStack.x = x + deltaX;
		newStack.y = y + deltaY;
		parent.addChild(newStack);
		KBlock.app.gh.grabOnMouseUp(newStack);
	}
...
//		var top:Block = topBlock();
//		// TODO: Remove any waiting reporter data in the KBlock.app.extensionManager
//		if (parent is Block) Block(parent).removeBlock(this);
...
//		if (top != this) x += top.width + 5;
//		removeComments();
//		KBlock.app.runtime.recordForUndelete(this, x, y, 0, KBlock.app.viewedObj());
//		KBlock.app.scriptsPane.saveScripts();
//		KBlock.app.runtime.checkForGraphicEffects();
//		return false;
	}
	private function removeComments():void{
		var comments:Array = attachedCommentsIn(KBlock.app.scriptsPane);
		...
		KBlock.app.scriptsPane.fixCommentLayout();
	}
	...
	public function objToGrab(evt:MouseEvent):Block {
		if (isEmbeddedParameter() || isInPalette()) return duplicate(false, KBlock.app.viewedObj() is ScratchStage);
		return this;
	}
	...
	public function click(evt:MouseEvent):void {
		...
		if(topBlock().op.indexOf("runArduino")>-1){
			if(!KBlock.app.stageIsArduino){
				KBlock.app.changeToArduinoMode();
			}
			return;
		}
		KBlock.app.runtime.interp.toggleThread(topBlock(), KBlock.app.viewedObj(), 1);
	}
