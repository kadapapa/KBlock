...
//		KBlock.app.interp.redraw();
	...
	private function addItem(b:IconButton = null):void {
		...
		if ((root is KBlock) && !(root as KBlock).editMode) return;
...
//		if (limitedCloudView &&
//			KBlock.app.isLoggedIn() && KBlock.app.editMode &&
//			(KBlock.app.projectOwner == KBlock.app.userName)) {
//				limitedCloudView = false; // only project owner can view cloud list contents
//		}
		var isEditable:Boolean = KBlock.app.editMode && !limitedView;
		updateElementCount();
		removeAllCells();
		visibleCells = [];
		visibleCellNums = [];
