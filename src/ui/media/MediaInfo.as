...
public class MediaInfo extends Sprite {
	...
	public function objToGrab(evt:MouseEvent):* {
		var result:MediaInfo = KBlock.app.createMediaInfo({
			type: objType,
			name: objName,
			md5: md5
		});
		if (mycostume) result = KBlock.app.createMediaInfo(mycostume, owner);
		if (mysound) result = KBlock.app.createMediaInfo(mysound, owner);
		if (mysprite) result = KBlock.app.createMediaInfo(mysprite);
		if (scripts) result = KBlock.app.createMediaInfo(scripts);
	...
	protected function duplicateMe():void {
		if (owner) {
			if (mycostume) KBlock.app.addCostume(mycostume.duplicate());
			if (mysound) KBlock.app.addSound(mysound.duplicate());
		}
	}

	protected function deleteMe(ignore:* = null):void {
		if (owner) {
			KBlock.app.runtime.recordForUndelete(this, 0, 0, 0, owner);
			if (mycostume) {
				owner.deleteCostume(mycostume);
				KBlock.app.refreshImageTab(false);
			}
			if (mysound) {
				owner.deleteSound(mysound);
				KBlock.app.refreshSoundTab()
			}
		}
	}
