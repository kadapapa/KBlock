...
public class ProjectIO {
	...
	protected var app:KBlock;
	protected var images:Array = [];
	protected var sounds:Array = [];

	public function ProjectIO(app:KBlock):void {
		this.app = app;
	}
	...
	private function decodeFromZipFile(zipData:ByteArray):ScratchObj {
		...
		if(jsonData.indexOf("PicoBoard")>-1){
			...
			if(!KBlock.app.extensionManager.checkExtensionSelected("Makeblock")){
				KBlock.app.extensionManager.onSelectExtension("Makeblock");
			}
		}else if(jsonData.indexOf("Arduino.")>-1){
			if(!KBlock.app.extensionManager.checkExtensionSelected("Arduino")){
				KBlock.app.extensionManager.onSelectExtension("Arduino");
			}
		}else if(jsonData.indexOf("ICbrick.")>-1){
			if(!KBlock.app.extensionManager.checkExtensionSelected("KBlock")){
				KBlock.app.extensionManager.onSelectExtension("KBlock");
			}
		}else if(jsonData.indexOf("Kadapapa.")>-1){
			if(!KBlock.app.extensionManager.checkExtensionSelected("KBlock")){
				KBlock.app.extensionManager.onSelectExtension("KBlock");
			}
		}
