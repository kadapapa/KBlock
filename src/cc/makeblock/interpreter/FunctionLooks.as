...
package cc.makeblock.interpreter {
	...
	private function primSceneIndex(thread:Thread, argList:Array):void {
		thread.push( KBlock.app.stagePane.costumeNumber());
	}

	private function primSceneName(thread:Thread, argList:Array):void {
		thread.push( KBlock.app.stagePane.currentCostume().costumeName);
	}

	private function startScene(s:String):Array {
		if ('next backdrop' == s) s = backdropNameAt(KBlock.app.stagePane.currentCostumeIndex + 1);
		else if ('previous backdrop' == s) s = backdropNameAt(KBlock.app.stagePane.currentCostumeIndex - 1);
		else {
			var n:Number = Interpreter.asNumber(s);
			if (!isNaN(n)) {
				n = (Math.round(n) - 1) % KBlock.app.stagePane.costumes.length;
				if (n < 0) n += KBlock.app.stagePane.costumes.length;
				s = KBlock.app.stagePane.costumes[n].costumeName;
			}
		}
		...
		var receivers:Array = [];
		KBlock.app.stagePane.showCostumeNamed(s);
		KBlock.app.runtime.allStacksAndOwnersDo(findSceneHats);
		var threadList:Array = [];
		for each(var item:Array in receivers){
			threadList.push(KBlock.app.interp.toggleThread(item[0], item[1]));
		}
		return threadList;
//		startAllReceivers(receivers, waitFlag);
	...
	private function backdropNameAt(i:int):String {
		var costumes:Array = KBlock.app.stagePane.costumes;
	...
	private function minSpriteLayer():int {
		...
		var stg:ScratchStage = KBlock.app.stagePane;
	...
	private function primSetVideoState(thread:Thread, argList:Array):void {
		KBlock.app.stagePane.setVideoState(argList[0]);
	}

	private function primSetVideoTransparency(thread:Thread, argList:Array):void {
		KBlock.app.stagePane.setVideoTransparency(Number(argList[0]));
		KBlock.app.stagePane.setVideoState('on');
	}
