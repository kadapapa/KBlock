...
//			runtime.interp.redraw();
	...
	private function specForCmd():String {
		...
		if(i > -1) {
			var spec:Array = KBlock.app.extensionManager.specForCmd(cmd);
	...
	private function getValue(runtime:ScratchRuntime):* {
		...
		if(index >= 0){
			...
			var ext:ScratchExtension = KBlock.app.extensionManager.extensionByName(extName);
...
/*
		if(cmd.indexOf('.') > -1) {
			...
			if(spec) {
				...
				return KBlock.app.interp.evalCmd(block);
			}
		}
*/
		return "unknown: " + cmd;
	...
	public function menu(evt:MouseEvent):Menu {
		function handleMenu(item:int):void {
			...
			if (item == 10) {
				visible = false;
				KBlock.app.updatePalette(false);
			}
		}
		if (!KBlock.app.editMode) return null;
...
//			m.addItem("text", 4);
	...
	private function mouseDown(evt:MouseEvent):void {
		...
		if (p.y > 20) KBlock(root).gh.setDragClient(this, evt);
	}
