package cc.makeblock.interpreter
{
	...
	internal class PrimInit
	{
		...
		{
			if(!KBlock.app.interp.turboMode){
...
//			if (target.activeThread.firstTime) {
			...
			KBlock.app.runtime.allStacksAndOwnersDo(findReceivers);
			var threadList:Array = [];
			for each(var item:Array in receivers){
				var newThread:Thread = KBlock.app.interp.toggleThread(item[0], item[1]);
