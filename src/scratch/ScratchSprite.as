...
//		var b:Rectangle = (!KBlock.app.render3D || currentCostume().bitmap) ? img.getChildAt(0).getBounds(this) : getVisibleBounds(this);
		...
		if(KBlock.app.isIn3D) {
			...
			updateEffects();
			var bm:BitmapData = KBlock.app.render3D.getRenderedChild(this, b.width*scaleX, b.height*scaleY);
...
//		}
	...
	public function handleTool(tool:String, evt:MouseEvent):void {
		...
		//if (tool == 'help') KBlock.app.showTip('scratchUI');
	}

	private function growSprite():void { setSize(getSize() + 5); KBlock.app.updatePalette() }
	private function shrinkSprite():void { setSize(getSize() - 5); KBlock.app.updatePalette() }
...
//			}
	...
	private function saveToLocalFile():void {
		function success():void {
			KBlock.app.log('sprite saved to file: ' + file.name);
		}
		var zipData:ByteArray = new ProjectIO(KBlock.app).encodeSpriteAsZipFile(copyToShare());
	...
	private function bubbleRect():Rectangle {
		...
		// compute bounds
		var p:Point = KBlock.app.stagePane.globalToLocal(localToGlobal(new Point(0, 0)));
		if (cachedBounds == null) bitmap(); // computes cached bounds
