...
public class ScratchCostume {
	...
	public function setBitmapData(bm:BitmapData, centerX:int, centerY:int):void {
		...
		rotationCenterX = centerX;
		rotationCenterY = centerY;
		if (KBlock.app && KBlock.app.viewedObj() && (KBlock.app.viewedObj().currentCostume() == this)) {
			KBlock.app.viewedObj().updateCostume();
			KBlock.app.refreshImageTab(true);
		}
	}
	...
	public function setSVGData(data:ByteArray, computeCenter:Boolean, fromEditor:Boolean = true):void {
		...
		function refreshAfterImagesLoaded():void {
			...
			if (KBlock.app && KBlock.app.viewedObj() && (KBlock.app.viewedObj().currentCostume() == thisC)) {
				KBlock.app.viewedObj().updateCostume();
				KBlock.app.refreshImageTab(fromEditor);
			}
			svgLoading = false;
		}
...
//trace('found bounds: '+new Rectangle(minX, minY, maxX - minX, maxY - minY));
	...
	public function bitmapForEditor(forStage:Boolean):BitmapData {
		...
		var oldQuality:String = KBlock.app.stage.quality;
		KBlock.app.stage.quality = StageQuality.LOW;
		bm.draw(dispObj, m);
		KBlock.app.stage.quality = oldQuality;
