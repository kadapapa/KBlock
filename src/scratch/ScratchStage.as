...
public class ScratchStage extends ScratchObj {
	...
	public function spritesAndClonesNamed(spriteName:String):Array {
		...
		var app:KBlock = parent as KBlock;
		if (app != null) {
...
//		if(KBlock.app.isIn3D) return KBlock.app.render3D.getUIContainer();
		return this;
	}

	override protected function updateImage():void {
		super.updateImage();
//		if(KBlock.app.isIn3D)
//			KBlock.app.render3D.getUIContainer().transform.matrix = transform.matrix.clone();
...
//			if(KBlock.app.isIn3D) KBlock.app.render3D.updateRender(videoImage);
		}
...
//			KBlock.app.render3D.updateRender(penLayer);
...
//		if(!KBlock.app.render3D || s.width < 1 || s.height < 1) return null;
		return null;
/*
		...
		var bmd:BitmapData = KBlock.app.render3D.getRenderedChild(s, b.width*s.scaleX, b.height*s.scaleY, for_carry);
	...
	public function setVideoState(newState:String):void {
		...
		KBlock.app.libraryPart.showVideoButton();
	...
	public function clearPenStrokes():void {
		...
		penActivity = false;
//		if(KBlock.app.isIn3D) KBlock.app.render3D.updateRender(penLayer);
	}
...
//		if(KBlock.app.isIn3D) KBlock.app.render3D.updateFilters(spr, effects);
	}
	...
	public function getBitmapWithoutSpriteFilteredByColor(s:ScratchSprite, c:int):BitmapData {
		...
		if(KBlock.app.isIn3D) {
			var b:Rectangle = s.currentCostume().bitmap ? s.img.getChildAt(0).getBounds(s) : s.getVisibleBounds(s);
			bm1 = KBlock.app.render3D.getOtherRenderedChildren(s, 1);
			//mask = 0x80F8F8F0;
		}
		else {*/
			// OLD code here
...
//		if(KBlock.app.isIn3D) KBlock.app.render3D.updateRender(dispObj, renderID, renderOpts);
	}
...
//		if(KBlock.app.isIn3D) KBlock.app.render3D.getRender(bm);
//		else
		bm.draw(this);
	...
	public function updateInfo():void {
		...
		if (KBlock.app.projectID != '') info.projectID = KBlock.app.projectID;
		info.videoOn = isVideoOn();
		info.swfVersion = KBlock.versionString;

		delete info.loadInProgress;
		if (KBlock.app.loadInProgress) info.loadInProgress = true; // log flag for debugging

		if (this == KBlock.app.stagePane) {
			// If this is the active stage pane, record the current extensions.
			var extensionsToSave:Array = KBlock.app.extensionManager.extensionsToSave();
	...
	public function handleDrop(obj:*):Boolean {
		if ((obj is ScratchSprite) || (obj is Watcher) || (obj is ListWatcher)) {
			...
			if (!(obj is ScratchSprite) || KBlock.app.editMode) KBlock.app.setSaveNeeded();
			return true;
		}
		KBlock.app.setSaveNeeded();
