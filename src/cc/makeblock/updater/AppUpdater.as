package cc.makeblock.updater
{
	...
	public class AppUpdater extends EventDispatcher
	{
		...
		static public const CONFIG_PATH:String = "http://www.kadapapa.com/?m=index&a=page&id=2";
...
//		private var frame:UpdateFrame;
		...
		*/
		...
		private function __onLoad(evt:Event):void
		{
			...
			if(isSourceVerGreatThan(result[1], KBlock.versionString.slice(1)) && (needNotice || SharedObjectManager.sharedManager().getObject(_key, true))){
		...
		private function __onConfirm(value:int):void
		{
			...
			if(value == JOptionPane.YES){
				navigateToURL(new URLRequest("http://www.kadapapa.com/?m=index&a=page&id=2"));
			}
			_checkBox = null;
//			closeAndNotify();
		}
		...
		{
			return File.documentsDirectory.resolvePath("KBlock/version.txt");
		}
		...
		private function runProcess(zipFile:File, callback:Function):void
		{
			...
			var wd:File = File.documentsDirectory.resolvePath("KBlock");
			if(!wd.exists){
				wd.createDirectory();
			}
