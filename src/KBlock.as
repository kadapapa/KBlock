package {
	...
	[SWF(frameRate="30")]
	public class KBlock extends Sprite {
		// Version
		...
		public static var app:KBlock; // static reference to the app, used for debugging
...
//			try{
				...
				var extensionsPath:File = ApplicationManager.sharedManager().documents.resolvePath("KBlock");
...
//			ClickerManager.sharedManager().update();
			SerialManager.sharedManager().setKBlock(this);
			HIDManager.sharedManager().setKBlock(this);
		}
...
//			openSwf("welcome.swf");
		...
		public function track(msg:String):void{
			ga.trackPageview(
				(ApplicationManager.sharedManager().isCatVersion?"/myh/":"/") + KBlock.versionString + msg
			);
		}
		...
		public function getMediaLibrary(app:KBlock, type:String, whenDone:Function):MediaLibrary {
			return new MediaLibrary(app, type, whenDone);
		}
	
		public function getMediaPane(app:KBlock, type:String):MediaPane {
		...
		private function onExiting(evt:Event):void{
			...
			KBlock.app.gh.mouseUp(new MouseEvent(MouseEvent.MOUSE_UP));
			SerialManager.sharedManager().disconnect();
			HIDManager.sharedManager().disconnect();
//			NativeApplication.nativeApplication.exit();
//			SerialManager.sharedManager().AIRSerial.dispose();
		}
...
//			function twoDigits(n:int):String { return ((n < 10) ? '0' : '') + n }
		...
		private function preDoRevert():void {
			revertUndo = new ProjectIO(KBlock.app).encodeProjectAsZipFile(stagePane);
			doRevert();
		}
