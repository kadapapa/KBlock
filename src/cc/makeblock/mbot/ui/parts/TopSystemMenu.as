...
///////////////////////////////////////
	...
	import cc.makeblock.mbot.ui.parts.HelpFrame
/////////////////////////////////////////////////////////////////	
...
//			getNativeMenu().getItemByName("Help").submenu.addEventListener(Event.DISPLAYING, __onShowLanguage);
		...
		private function __onFile(item:NativeMenuItem):void
		{
			...
			{
				case "New":
					KBlock.app.createNewProject();
					break;
				case "Load Project":
					KBlock.app.runtime.selectProjectFile();
					break;
				case "Save Project":
					KBlock.app.saveFile();
					break;
				case "Save Project As":
					KBlock.app.exportProjectToFile();
					break;
				case "Open Example":
					KBlock.app.runtime.example();
					break;
				case "Undo Revert":
					KBlock.app.undoRevert();
					break;
				case "Revert":
					KBlock.app.revertToOriginalProject();
					break;
				case "Import Image":
		...
		private function __onEdit(item:NativeMenuItem):void
		{
			switch(item.name){
				case "Undelete":
					KBlock.app.runtime.undelete();
					break;
				case "Hide stage layout":
					KBlock.app.toggleHideStage();
					break;
				case "Small stage layout":
					KBlock.app.toggleSmallStage();
					break;
				case "Turbo mode":
					KBlock.app.toggleTurboMode();
					break;
				case "Offline mode":
					KBlock.app.changeToArduinoMode();
					break;
			}
			KBlock.app.track("/OpenEdit");
		}
		...
		private function __onInitFielMenu(evt:Event):void
		{
			...
			MenuUtil.setEnable(menu.getItemByName("Undo Revert"), KBlock.app.canUndoRevert());
			MenuUtil.setEnable(menu.getItemByName("Revert"), KBlock.app.canRevert());
			
			KBlock.app.track("/OpenFile");
		}
		
		private function __onInitEditMenu(evt:Event):void
		{
			var menu:NativeMenu = evt.target as NativeMenu;
			MenuUtil.setEnable(menu.getItemByName("Undelete"), KBlock.app.runtime.canUndelete());
			MenuUtil.setChecked(menu.getItemByName("Hide stage layout"), KBlock.app.stageIsHided);
			MenuUtil.setChecked(menu.getItemByName("Small stage layout"), !KBlock.app.stageIsHided && KBlock.app.stageIsContracted);
			MenuUtil.setChecked(menu.getItemByName("Turbo mode"), KBlock.app.interp.turboMode);
			MenuUtil.setChecked(menu.getItemByName("Offline mode"), KBlock.app.stageIsArduino);
			KBlock.app.track("/OpenEdit");
		}
		...
		private function __onShowConnect(evt:Event):void
		{
			...
			var enabled:Boolean = KBlock.app.extensionManager.checkExtensionEnabled();
...
//			menuItem.addEventListener(evt.type, __onShowExtMenu);
			var list:Array = KBlock.app.extensionManager.extensionList;
			if(list.length==0){
				KBlock.app.extensionManager.copyLocalFiles();
			...
			list = KBlock.app.extensionManager.extensionList;
//			var subMenu:NativeMenu = menuItem;
			for(var i:int=0;i<list.length;i++){
				...
				subMenuItem.checked = KBlock.app.extensionManager.checkExtensionSelected(extName);
		...
		*/
		...
		{
			KBlock.app.extensionManager.onSelectExtension(menuItem.name);
		}		

		
		private var _help:HelpFrame = new HelpFrame();
		private function __onHelp(menuItem:NativeMenuItem):void
		{
			...
			KBlock.app.track("/OpenHelp/"+menuItem.data.@key);
			switch(menuItem.data.@key.toString()){
				...
				case "work share":
					break;
				case "manual":
					var _manual:File = new File(File.applicationDirectory.nativePath+"/help/KBlock User Manual.pdf");
					_manual.openWithDefaultApplication();
					break;
				case "support":
					_help.MyHelp("support");
					break;
				case "check for updates":
//					AppUpdater.getInstance().start(true);
					break;
				case "acknowledgements":
					_help.MyHelp("acknowledgements");
					break;
				case "about":
					_help.MyHelp("about");
					break;	
				case "suggest":
					_help.MyHelp("suggest");
					break;
				/*
...
/*			
			...
			{
				case "Work Share":
					KBlock.app.track("/OpenShare/");
					break;
				case "FAQ":
					KBlock.app.track("/OpenFaq/");
					break;
				default:
					KBlock.app.track("/OpenHelp/"+menuItem.data.@key);
			}
